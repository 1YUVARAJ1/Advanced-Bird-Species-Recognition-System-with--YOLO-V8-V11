...
def get_yolo_model(model_name):
    if model_name not in MODEL_REGISTRY:
        ...
        MODEL_REGISTRY[model_name] = YOLO(model_name)
...
class BirdRecognitionPipeline:
    ...
    def detect_bird(self, image_path_or_array, yolo_model_name, device_name):
        """ Run YOLO detection and return highest confidence bird bbox """
        detector = get_yolo_model(yolo_model_name)
        results = detector(image_path_or_array, classes=[14], verbose=False, device=device_name) # class 14 is 'bird' in COCO dataset
    ...
    def run_pipeline(self, image_path, yolo_model_name='yolo11n.pt', device_name='cpu'):
        ...
        # 1. Detect
        det_start = time.time()
        bbox, yolo_conf = self.detect_bird(image_path, yolo_model_name, device_name)
